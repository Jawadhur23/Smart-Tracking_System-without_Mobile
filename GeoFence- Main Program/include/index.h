...
<html>
  ...
  <body style="background-color: #ffffff ; font-family:verdana">
    ...
    <center>
      ...
      <hr style = "width: 70%;">
      <div id="map"></div>
        <script async src="https://maps.googleapis.com/maps/api/js?key=AIzaSyD4oyfwEbnJ9K8Zb8UmdaG_J0lfYVK1Yrc&callback=initMap"></script>
      <script>
    ...
    var options={
        center:{lat:19.0645,lng:72.8359},
        zoom:18
    }
    map=new google.maps.Map(document.getElementById("map"),options)
    

var lat_val = Number(document.getElementById("lat_val").innerHTML);
var lng_val = Number(document.getElementById("lon_val").innerHTML);
document.write(lat_val);
document.write(lon_val);
function addMarker(property){
    const marker = new google.maps.Marker({
        position:property.location,
        map:map,
        icon:"https://img.icons8.com/glyph-neue/100/man-in-a-tuxedo.png"
    });
}

    addMarker({location:{lat:19.0645,lng:72.8359}});
}
      </script>
    </center>
  </body>
